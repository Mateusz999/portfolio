...
editLinks.forEach( (el) => {
    ...
    el.addEventListener("click", (event) => {
        ...
        fetch('/admin-secret-key/get-article/'+ article_id)
            .then( (res) => {
                if(res.ok) return res.json() //zamiana danych w jsona
            })
            .then( (response) => {
                if(response){
                    ...
                    saveArticleForm.elements.namedItem('category').
                    setAttribute('disabled', 'disabled')
                    saveArticleForm.elements.namedItem('created_at').value =
                     new Date(response.created_at).toISOString().split('T')[0]
                   // console.log(new Date(response.created_at).toISOString().split('T')[0])
